...
def upgrade() -> None:
    op.add_column("book_assets", sa.Column("asset_role", sa.String(length=30), nullable=True))
    op.execute("UPDATE book_assets SET asset_role = 'access' WHERE asset_role IS NULL")
    op.alter_column("book_assets", "asset_role", nullable=False, server_default="access")
    ...
    op.create_index("ix_book_assets_asset_role", "book_assets", ["asset_role"], unique=False)
...
def downgrade() -> None:
    ...
    if master_count:
        raise RuntimeError("Cannot downgrade asset roles while preservation-master assets exist")
    ...
    """)).scalar_one()
    if duplicate_count:
        raise RuntimeError("Cannot downgrade asset roles because asset versions would collide")
    ...
    op.create_check_constraint(
        "ck_book_asset_role", "book_assets", "asset_role IN ('preservation_master', 'access')"
    )
    op.create_unique_constraint(
        "uq_book_asset_version",
        "book_assets",
