...
def to_library_item_read(row: LibraryItem) -> LibraryItemRead:
    return LibraryItemRead(
        ...
        book=BookRead(
            ...
            content_type=row.book.content_type,
            source_url=row.book.source_url,
            mime_type=row.book.mime_type,
            visibility=getattr(row.book, "visibility", None) or "published",
            archived_at=getattr(row.book, "archived_at", None),
            cover_path=getattr(row.book, "cover_path", None),
            is_featured=bool(getattr(row.book, "is_featured", False)),
        ),
    )
